...
static const char *kMaxVersionStr = CHECKSUMHELPER_VERSION_STR_PREFIX CHECKSUMHELPER_MACRO_VAL_TO_STR(
        CHECKSUMHELPER_MAX_VERSION);
...
void ChecksumCalculator::save(IOStream *stream) {
...
void ChecksumCalculator::load(IOStream *stream) {
    assert(!m_isEncodingChecksum);
    switch (m_version) {
        case 1:
            assert(m_v1BufferTotalLength == 0);
