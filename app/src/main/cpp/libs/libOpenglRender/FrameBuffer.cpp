...
#include "libs/GLESv2_dec/gles2_dec.h"
...
bool FrameBuffer::removeSubWindow_locked() {
    ...
    if (m_subWin) {
        ...
        m_subWin = (EGLNativeWindowType) nullptr;
        removed = true;
    }
    return removed;
}
...
HandleType FrameBuffer::createRenderContext(int p_config,
    ...
    ret = genHandle_locked();
    ALOGD("xxx p_config = %d", p_config);
    RenderContextPtr rctx(RenderContext::create(
            m_eglDisplay,
//            p_config ? config->getEglConfig() : ((EGLConfig) 0),
            config->getEglConfig(),
            sharedContext, ret,
            version));
    if (rctx.get() != NULL) {
        ALOGD("xxx create context: p_config = %d", p_config);
        m_contexts[ret] = rctx;
        RenderThreadInfo *tinfo = RenderThreadInfo::get();
        uint64_t puid = tinfo->m_puid;
...
HandleType FrameBuffer::createWindowSurface(int p_config,
    ...
    if (win.get() != NULL) {
        ALOGD("xxx create window : p_config = %d", p_config);
        m_windows[ret] = {win, 0};
        RenderThreadInfo *tInfo = RenderThreadInfo::get();
        uint64_t puid = tInfo->m_puid;
        if (puid) {
...
bool FrameBuffer::bindContext(HandleType p_context,
    ...
    WindowSurfacePtr draw = nullptr, read = nullptr;
...
void FrameBuffer::doPostCallback(void *pixels) {
    m_onPost(m_onPostContext, m_framebufferWidth, m_framebufferHeight, -1, GL_RGBA,
             GL_UNSIGNED_BYTE,
