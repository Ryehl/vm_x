    ...
    int ReadBuffer::getData(IOStream *stream, int minSize) {
        ...
        if (freeTailSize >= minSizeToRead) {
            ...
            if (freeTailSize + (m_readPtr - m_buf) >= minSizeToRead) {
                ...
                if (!new_buf) {
                    ERR("xxx Failed to alloc %zu bytes for ReadBuffer\n", new_size);
                    return -1;
                }
        ...
        int readTotal = 0;
        do {
            size_t b = maxSizeToRead - readTotal;
            size_t readNow = stream->recv(m_readPtr + m_validData, b);
            if (!readNow) {
                if (readTotal > 0) {
                    return readTotal;
                } else {
        ...
        if (readTotal < 0) {
            ERR("xxx ReadBuffer getData error: %d, msg: %s", errno, strerror(errno));
        }
