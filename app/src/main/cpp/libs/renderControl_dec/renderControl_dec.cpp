...
#  define DEBUG(...) do { ALOGE(__VA_ARGS__); } while(0)
#else
#  define DEBUG(...)  ((void)0)
#endif
    ...
    while (end - ptr >= 8) {
        ...
        switch (opcode) {
            ...
            case OP_rcGetEGLVersion: {
                ...
                uint32_t size_minor __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4);
                if (useChecksum) {
                ...
                DEBUG("renderControl(%p): rcGetEGLVersion(%p(%u) %p(%u) )\n", stream,
                      (EGLint *) (outptr_major.get()), size_major, (EGLint *) (outptr_minor.get()),
                      size_minor);
                *(EGLint *) (&tmpBuf[0 + size_major + size_minor]) = this->rcGetEGLVersion(
                        (EGLint *) (outptr_major.get()), (EGLint *) (outptr_minor.get()));
                outptr_major.flush();
                outptr_minor.flush();
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
            ...
            case OP_rcQueryEGLString: {
                ...
                uint32_t size_buffer __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4);
                ...
                DEBUG("renderControl(%p): rcQueryEGLString(0x%08x %p(%u) 0x%08x )\n", stream,
                      var_name, (void *) (outptr_buffer.get()), size_buffer, var_bufferSize);
                *(EGLint *) (&tmpBuf[0 + size_buffer]) = this->rcQueryEGLString(var_name,
                                                                                (void *) (outptr_buffer.get()),
                                                                                var_bufferSize);
                outptr_buffer.flush();
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
            ...
            case OP_rcGetGLString: {
                ...
                DEBUG("renderControl(%p): rcGetGLString(0x%08x %p(%u) 0x%08x )\n", stream, var_name,
                      (void *) (outptr_buffer.get()), size_buffer, var_bufferSize);
                *(EGLint *) (&tmpBuf[0 + size_buffer]) = this->rcGetGLString(var_name,
                                                                             (void *) (outptr_buffer.get()),
                                                                             var_bufferSize);
                outptr_buffer.flush();
                if (useChecksum) {
            ...
            case OP_rcGetNumConfigs: {
                uint32_t size_numAttribs __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8);
                if (useChecksum) {
                ...
                DEBUG("renderControl(%p): rcGetNumConfigs(%p(%u) )\n", stream,
                      (uint32_t *) (outptr_numAttribs.get()), size_numAttribs);
                *(EGLint *) (&tmpBuf[0 + size_numAttribs]) = this->rcGetNumConfigs(
                        (uint32_t *) (outptr_numAttribs.get()));
            ...
            case OP_rcGetConfigs: {
                ...
                DEBUG("renderControl(%p): rcGetConfigs(0x%08x %p(%u) )\n", stream, var_bufSize,
                      (GLuint *) (outptr_buffer.get()), size_buffer);
                *(EGLint *) (&tmpBuf[0 + size_buffer]) = this->rcGetConfigs(var_bufSize,
                                                                            (GLuint *) (outptr_buffer.get()));
                outptr_buffer.flush();
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcChooseConfig: {
                ...
                uint32_t size_configs __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + size_attribs + 4);
                uint32_t var_configs_size = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + size_attribs + 4 + 4);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::validOrDie(checksumCalc, ptr,
                                                             8 + 4 + size_attribs + 4 + 4 + 4,
                                                             ptr + 8 + 4 + size_attribs + 4 + 4 + 4,
                                                             checksumSize,
                ...
                DEBUG("renderControl(%p): rcChooseConfig(%p(%u) 0x%08x %p(%u) 0x%08x )\n", stream,
                      (EGLint *) (inptr_attribs.get()), size_attribs, var_attribs_size,
                      (uint32_t *) (outptr_configs.get()), size_configs, var_configs_size);
                *(EGLint *) (&tmpBuf[0 + size_configs]) = this->rcChooseConfig(
                        (EGLint *) (inptr_attribs.get()), var_attribs_size,
                        size_configs == 0 ? nullptr : (uint32_t *) (outptr_configs.get()),
                        var_configs_size);
            ...
            case OP_rcCreateContext: {
                ...
                DEBUG("renderControl(%p): rcCreateContext(0x%08x 0x%08x 0x%08x )\n", stream,
                      var_config, var_share, var_glVersion);
                *(uint32_t *) (&tmpBuf[0]) = this->rcCreateContext(var_config, var_share,
                                                                   var_glVersion);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcCreateWindowSurface: {
                ...
                DEBUG("renderControl(%p): rcCreateWindowSurface(0x%08x 0x%08x 0x%08x )\n", stream,
                      var_config, var_width, var_height);
                *(uint32_t *) (&tmpBuf[0]) = this->rcCreateWindowSurface(var_config, var_width,
                                                                         var_height);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcCreateColorBuffer: {
                ...
                DEBUG("renderControl(%p): rcCreateColorBuffer(0x%08x 0x%08x 0x%08x )\n", stream,
                      var_width, var_height, var_internalFormat);
                *(uint32_t *) (&tmpBuf[0]) = this->rcCreateColorBuffer(var_width, var_height,
                                                                       var_internalFormat);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcSetWindowColorBuffer: {
                ...
                DEBUG("renderControl(%p): rcSetWindowColorBuffer(0x%08x 0x%08x )\n", stream,
                      var_windowSurface, var_colorBuffer);
            ...
            case OP_rcMakeCurrent: {
                ...
                DEBUG("renderControl(%p): rcMakeCurrent(0x%08x 0x%08x 0x%08x )\n", stream,
                      var_context, var_drawSurf, var_readSurf);
                *(EGLint *) (&tmpBuf[0]) = this->rcMakeCurrent(var_context, var_drawSurf,
                                                               var_readSurf);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcColorBufferCacheFlush: {
                ...
                DEBUG("renderControl(%p): rcColorBufferCacheFlush(0x%08x 0x%08x %d )\n", stream,
                      var_colorbuffer, var_postCount, var_forRead);
                *(EGLint *) (&tmpBuf[0]) = this->rcColorBufferCacheFlush(var_colorbuffer,
                                                                         var_postCount,
                                                                         var_forRead);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
            ...
            case OP_rcReadColorBuffer: {
                ...
                DEBUG("renderControl(%p): rcReadColorBuffer(0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x %p(%u) )\n",
                      stream, var_colorbuffer, var_x, var_y, var_width, var_height, var_format,
                      var_type, (void *) (outptr_pixels.get()), size_pixels);
                this->rcReadColorBuffer(var_colorbuffer, var_x, var_y, var_width, var_height,
                                        var_format, var_type, (void *) (outptr_pixels.get()));
                outptr_pixels.flush();
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcUpdateColorBuffer: {
                ...
                DEBUG("renderControl(%p): rcUpdateColorBuffer(0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x %p(%u) )\n",
                      stream, var_colorbuffer, var_x, var_y, var_width, var_height, var_format,
                      var_type, (void *) (inptr_pixels.get()), size_pixels);
                *(int *) (&tmpBuf[0]) = this->rcUpdateColorBuffer(var_colorbuffer, var_x, var_y,
                                                                  var_width, var_height, var_format,
                                                                  var_type,
                                                                  (void *) (inptr_pixels.get()));
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
                                                                &tmpBuf[totalTmpSize -
            ...
            case OP_rcCreateClientImage: {
                ...
                DEBUG("renderControl(%p): rcCreateClientImage(0x%08x 0x%08x 0x%08x )\n", stream,
                      var_context, var_target, var_buffer);
                *(uint32_t *) (&tmpBuf[0]) = this->rcCreateClientImage(var_context, var_target,
                                                                       var_buffer);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcCreateSyncKHR: {
                ...
                uint32_t size_attribs __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4);
                InputBuffer inptr_attribs(ptr + 8 + 4 + 4, size_attribs);
                uint32_t var_num_attribs = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + 4 + size_attribs);
                int var_destroy_when_signaled = Unpack<int, uint32_t>(
                        ptr + 8 + 4 + 4 + size_attribs + 4);
                uint32_t size_glsync_out __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + 4 + size_attribs + 4 + 4);
                uint32_t size_syncthread_out __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + 4 + size_attribs + 4 + 4 + 4);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::validOrDie(checksumCalc, ptr,
                                                             8 + 4 + 4 + size_attribs + 4 + 4 + 4 +
                                                             4, ptr + 8 + 4 + 4 + size_attribs + 4 +
                                                                4 + 4 + 4, checksumSize,
                ...
                OutputBuffer outptr_syncthread_out(&tmpBuf[0 + size_glsync_out],
                                                   size_syncthread_out);
                DEBUG("renderControl(%p): rcCreateSyncKHR(0x%08x %p(%u) 0x%08x %d %p(%u) %p(%u) )\n",
                      stream, var_type, (EGLint *) (inptr_attribs.get()), size_attribs,
                      var_num_attribs, var_destroy_when_signaled,
                      (uint64_t *) (outptr_glsync_out.get()), size_glsync_out,
                      (uint64_t *) (outptr_syncthread_out.get()), size_syncthread_out);
                this->rcCreateSyncKHR(var_type, (EGLint *) (inptr_attribs.get()), var_num_attribs,
                                      var_destroy_when_signaled,
                                      (uint64_t *) (outptr_glsync_out.get()),
                                      (uint64_t *) (outptr_syncthread_out.get()));
            ...
            case OP_rcClientWaitSyncKHR: {
                ...
                DEBUG("renderControl(%p): rcClientWaitSyncKHR(0x%016lx 0x%08x 0x%016lx )\n", stream,
                      var_sync, var_flags, var_timeout);
                *(EGLint *) (&tmpBuf[0]) = this->rcClientWaitSyncKHR(var_sync, var_flags,
                                                                     var_timeout);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
                                                                totalTmpSize - checksumSize,
            ...
            case OP_rcUpdateColorBufferDMA: {
                ...
                GLenum var_format = Unpack<GLenum, uint32_t>(ptr + 8 + 4 + 4 + 4 + 4 + 4);
                GLenum var_type = Unpack<GLenum, uint32_t>(ptr + 8 + 4 + 4 + 4 + 4 + 4 + 4);
                uint64_t var_pixels_guest_paddr = Unpack<uint64_t, uint64_t>(
                        ptr + 8 + 4 + 4 + 4 + 4 + 4 + 4 + 4);
                //TODO dma used pipe
//			void* var_pixels = stream->getDmaForReading(var_pixels_guest_paddr);
                uint32_t size_pixels __attribute__((unused)) = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + 4 + 4 + 4 + 4 + 4 + 4 + 8);
                uint32_t var_pixels_size = Unpack<uint32_t, uint32_t>(
                        ptr + 8 + 4 + 4 + 4 + 4 + 4 + 4 + 4 + 8);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::validOrDie(checksumCalc, ptr,
                                                             8 + 4 + 4 + 4 + 4 + 4 + 4 + 4 + 8 + 4,
...
//                      stream, var_colorbuffer, var_x, var_y, var_width, var_height, var_format,
//                      var_type, var_pixels, var_pixels_size);
                *(int *) (&tmpBuf[0]) = this->rcUpdateColorBufferDMA(var_colorbuffer, var_x, var_y,
...
//			stream->unlockDma(var_pixels_guest_paddr);
            ...
            case OP_rcCreateColorBufferDMA: {
                ...
                *(uint32_t *) (&tmpBuf[0]) = this->rcCreateColorBufferDMA(var_width, var_height,
                                                                          var_internalFormat,
                                                                          var_frameworkFormat);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0],
            ...
            case OP_rcWaitSyncKHR: {
                ...
                EGLint var_flags = Unpack<EGLint, uint32_t>(ptr + 8 + 8);
                if (useChecksum) {
                    ChecksumCalculatorThreadInfo::validOrDie(checksumCalc, ptr, 8 + 8 + 4,
                                                             ptr + 8 + 8 + 4, checksumSize,
...
#if LOG_CALL_NAME
        if (strlen(lastCall))
            ALOGE("xxx s_egl call %s", lastCall);
#endif
        EGLint err = eglGetError();
        if (err != EGL_SUCCESS) {
            ALOGE("xxx s_egl code = %d, err = %d", opcode, err);
        }
#endif
