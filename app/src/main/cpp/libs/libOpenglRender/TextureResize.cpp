...
GLuint TextureResize::update(GLuint texture) {
    ...
    if (error != GL_NO_ERROR) {
        ALOGV("GL error while resizing: 0x%x (ignored)\n", error);
        return texture;
    }
